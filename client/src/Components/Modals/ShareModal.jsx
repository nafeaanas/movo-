...
function ShareMovieModal({modalOpen, setModalOpen, movie}) {
  ...
  return (
    <MainModal modalOpen={modalOpen} setModalOpen={setModalOpen}>
        <div className='inline-block sm:w-4/5 border border-border md:w-3/5 lg:w-2/5 w-full align-middle p-10 overflow-auto h-full bg-main text-white overflow-y-auto rounded-2xl'>
            ...
            <form className='flex-rows flex-wrap gap-6 mt-6'>
                {
                    shareData.map((data, index) => (
                        <data.shareButton key={index} url={url} quote='Movo | Free Movies Site'>
